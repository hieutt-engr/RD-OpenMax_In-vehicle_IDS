...
def main():
    ...
    # Khởi tạo dataset
    train_dataset = CANDataset(root_dir=args.data_folder, window_size=32, is_train=True, transform=train_transform)
    close_set_test_dataset = CANDataset(root_dir=args.close_set_test_data, window_size=32, is_train=False, transform=transform)

    open_set_test_dataset = CANDataset(root_dir=args.open_set_test_data, window_size=32, is_train=False, transform=transform)
    open_set_test_loader = torch.utils.data.DataLoader(open_set_test_dataset, batch_size=args.batch_size, shuffle=False,  pin_memory=True)

    dataloader_on = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)
    dataloader_off = torch.utils.data.DataLoader(close_set_test_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True, drop_last=True)
